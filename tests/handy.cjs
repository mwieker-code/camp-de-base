   ...
   angeht. */
{
  ...
  const t=seite('year7',390,true,{'fr:tastenprobe':'ios'});
   ...
   Sie blieb dadurch als einzige bei der Systemtastatur. */
{
  ...
  assert.equal(feldchen.dataset.sprache,'fr','Blitzrunde: es wird immer französisch geantwortet');
  ...
  console.log('Blitzrunde: eigenes Tastenfeld, französische Belegung, Prüfen-Taste');
}
...
for(const folder of ['year7','year8','year9','year10']){
  ...
  assert.equal(feldchen.dataset.sprache,'fr',folder+': falsche Belegung bei DE -> FR');
  /* Die Akzentreihe steht obenauf, das Apostroph neben dem l. */
  assert.ok(feldchen.querySelector('.tr.akzente .tk[data-k="é"]'),folder+': keine Akzentreihe');
  assert.ok(feldchen.querySelector('.tk[data-k="\'"]'),folder+': kein Apostroph');
  assert.ok(!t.d.querySelector('.akzentleiste'),folder+': Akzentleiste des Rechners neben dem Tastenfeld');
