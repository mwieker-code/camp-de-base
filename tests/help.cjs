...
for(const folder of ['year7','year8','year9','year10']){
 ...
 if(!irregular){
  for(const phrase of ['Unité, Französisch, Deutsch','höchstens zehn','Später fortsetzen','Standardmäßig übst du DE → FR','als PDF speichern','Zum Home-Bildschirm','Fortschritt sichern','hörst du einen kurzen Ton'])assert(text.includes(phrase),folder+': '+phrase);
  ...
  const ERWARTET=['Wofür ist diese Seite?','Loslegen','Die vier Übungsarten',
    ...
    'Camp de Base auf den Startbildschirm','Ohne Internet üben',
