...
d.querySelector('#promptAB').checked=true;const sentences=d.querySelector('[value=sentences]');sentences.checked=true;sentences.dispatchEvent(new w.Event('change'));d.querySelector('#promptTense').value='passé composé';d.querySelector('#promptCount').value=12;
for(const [t,n] of [['gap',5],['matching',4],['sentences',3]])d.querySelector('[data-allocation='+t+']').value=n;
submit();let p=d.querySelector('#promptText').value;assert(p.includes('vier getrennte'));assert(p.includes('Lückentext: 5; Übersetzen DE → FR: 4; Sätze bilden: 3'));assert(p.includes('Zeitform passé composé'));assert(p.includes('Total : / 12'));assert(!p.includes('Erstelle zwei getrennte'));
