...
function starten(gespeichert){
  ...
  const w=dom.window;
  if(gespeichert!==undefined) w.localStorage.setItem('fr:feedback:v1',gespeichert);
...
/* Ein Klick schaltet um und legt die Wahl ab. */
{
  ...
  const abgelegt=JSON.parse(t.w.localStorage.getItem('fr:feedback:v1'));
   ...
   Bleibt davon etwas stehen, gibt es wieder zwei Wege zum selben Ton. */
{
  ...
  assert.ok(seiten.length>=4,'zu wenige Seiten gefunden: '+seiten.length);
